...
namespace TFE_DarkForces
{
	...
	Tick defaultAttackFunc(ActorModule* module, MovementModule* moveMod)
	{
		...
		switch (state)
		{
			...
			case STATE_DECIDE:
			{
				...
				{
					bool vanillaDF = !TFE_Settings::enhancedAiLogics();		// vanilla code used s_eyePos for these calculations (enemies would target the eye rather than the player)
			...
			case STATE_ATTACK1:
			{
				...
				bool vanillaDF = !TFE_Settings::enhancedAiLogics();
				fixed16_16 targetY;
				if (logic->targetObject == s_playerObject)
				{
				...
				{
					targetY = logic->targetObject->posWS.y - logic->targetObject->worldHeight + ONE_16;	// AI - aim 1 unit lower than the head
				}
	...
	// Note: Dispatch actors in vanilla DF would attack the EYE object rather than the PLAYER object. This behaviour is 
	// preserved when the Enhanced AI setting is disabled. When the Enhanced AI setting is enabled, actors will target the
	// PLAYER object, not the EYE object.
	SecObject* findNewTargetObject(SecObject* sourceObj, s32 sourceTeam)
	{
		// If Enhanced AI setting is disabled, the target must always be the player (vanilla behaviour)
		if (!TFE_Settings::enhancedAiLogics())
		{
