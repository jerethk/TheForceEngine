...
namespace TFE_DarkForces
{
	...
	{
		s_playerWeaponList[id].frameCount = extWeapons[id].frameCount;
		s_playerWeaponList[id].frame = 0;
		
		for (s32 f = 0; f < s_playerWeaponList[id].frameCount; f++)
		{
			s_playerWeaponList[id].frames[f] = loadWeaponTexture(extWeapons[id].textures[f]);
	...
	void weapon_draw(u8* display, DrawRect* rect)
	{
		...
		if (s_wearingGasmask)
		{
			s32 x = TFE_ExternalData::getExternalGasmask()->xPos;
			s32 y = TFE_ExternalData::getExternalGasmask()->yPos;
			if (weapon)
			{
